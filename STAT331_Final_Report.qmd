...
First, we will analyze the Linear Regression assumptions.
...
Whether our errors are independent of each other depends on the sampling method of the data. Here we assume that the data from Gapminder was randomly sampled, and therefore the independence assumption is met.
...
model |>
  ...
  theme_bw()
```
In this plot of fitted values versus residuals, we can see some fanning in the data points. This indicates that the equal variance assumption is violated. Due to this violation, we will be cautious as we perform simple linear regression.
...
geom_point(aes(y = predicted_er), color = "darkseagreen3", shape=1) + 
...
<<<<<<< HEAD
Ideally, we would want the center of the distribution to fall near 1. An $R^2$ value of 1 would indicate that our simulated data explains 100% of the variability in our observed data, meaning it is a perfect fit. In the distribution we can see that the majorty of the $R^2$ values fall 0.125. This means that over the 1000 simulations, the majority of our generated data explained 12.5% of the variability in the observed data. This means that our model is not a great fit.
=======

Ideally, we would want the center of our distribution to fall near 1. An $R^2$ value of 1 would indicate that our simulated data explains 100% of the variability in our observed data, meaning it is a perfect fit. In our distribution we can see that the majority of the $R^2$ values fall 0.125. This means that over the 1000 simulations, the majority of our generated data explained 12.5% of the variability in the observed data. This means that our model is not a great fit.
>>>>>>> 033496fecae00429c42bfb74bbe2d6dbc4cdf676
