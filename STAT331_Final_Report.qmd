...
We will begin by investigating visualization of data on Gross Domestic Product and Fertility rate. Firstly, we investigate the relationship of Gross Domestic Product over time and Fertility rate over time. We average GDP and fertility across all countries per year, to get a single value for average GDP and average Fertility Rate for every year. We also normalized the values of GDP and fertility rate to be between 0 and 1, so that we can compare the two variables with each other, in the following graphic.
...
From our plot of average GDP vs. average Fertility in section 2.1, we can see that the data may not follow a linear relationship. The form of the observations seems to be more curved, and therefore the linearity assumption may be violated.

### Independence Assumptions
Whether out errors are independent of each other depends on the sampling method of the data. Here we assume that the data from Gapminder was randomly sampled, and therefore the independence assumption is met.
...
error_df|>
  ...
  geom_histogram()
```
This plot somewhat resembles a bell-curve. There are a few values that are unusually large, and they may be outliers. Nonetheless, we will concluded that the normality assumption is reasonably met.
...
```
Here we can see some fanning in the data points, which would indicate that the equal variance assumption may be violated.

# 3 Simulation

We now generate a predictive check, to assess how well our model is performing. We simulate data from our linear regression model and then compare this to the actual observed data points, reporting the proportion of variability explained. 

## 3.1 Visualizing Simulation from the Model

In the following graphic, we visualize a single simulation of data from our model next to our actual observations.
```{r}
#| code-fold: true
set.seed(100)
...
geom_point(aes(y = pred_er), color = "darkseagreen3", shape=1)+
  labs(title = "Simulated Data",
       ...
       y= NULL,
       subtitle = "Average Fertility (# of babies per woman)")
...
Next, we perform the same comparison as above 1000 times and report the proportion of variability explained in the observed data by our simulated data. Then, we can plot a distribution of this amount of variability explained across the 1000 simulations. This distribution is shown below.
...
```
Ideally, we would want the center of our distribution to fall near 1. An $R^2$ value of 1 would indicate that our simulated data explains 100% of the variability in our observed data, meaning it is a perfect fit. In our distribution we can see that the majorty of the $R^2$ values fall 0.125. This means that over the 1000 simulations, the majority of our generated data explained 12.5% of the variability in the observed data. This means that our model is not a great fit.

Our linear regression being a poor model may have been expected, as the data does not meet all the linear regression requirements (identified in section 2.2).
